...
class ModchartState 
{
	...
	{				var olddadx = PlayState.opponent.x;
					var olddady = PlayState.opponent.y;
					PlayState.instance.removeObject(PlayState.opponent);
					PlayState.opponent = new Character(olddadx, olddady, id);
					PlayState.instance.addObject(PlayState.opponent);
					PlayState.instance.iconP2.animation.play(id);
	}

	function changeBoyfriendCharacter(id:String)
	{				var oldboyfriendx = PlayState.player.x;
					var oldboyfriendy = PlayState.player.y;
					PlayState.instance.removeObject(PlayState.player);
					PlayState.player = new Boyfriend(oldboyfriendx, oldboyfriendy, id);
	...
	function makeLuaSprite(spritePath:String,toBeCalled:String, drawBehind:Bool)
	{
        ...
        @:privateAccess
        {
            if (drawBehind)
            {
                PlayState.instance.removeObject(PlayState.spectator);
                PlayState.instance.removeObject(PlayState.player);
                PlayState.instance.removeObject(PlayState.opponent);
            }
            PlayState.instance.addObject(sprite);
            if (drawBehind)
            {
                PlayState.instance.addObject(PlayState.spectator);
                PlayState.instance.addObject(PlayState.player);
                PlayState.instance.addObject(PlayState.opponent);
            }
        }
    ...
    function new()
    {
				...
				var result = LuaL.dofile(lua, Paths.lua(StringTools.replace( PlayState.SONG.song , " ", "-").toLowerCase() + "/modchart")); // execute der file				
				...
				// get some funkin globals up in here bois
