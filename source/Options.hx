...
class OffsetMenu extends Option
{
	...
	{
		trace("switch");
		var poop:String = Highscore.formatSong("Warmup", 1);

		PlayState.SONG = Song.loadFromJson(poop, "Warmup");
...
class DialogueControl extends Option
{
	public function new(desc:String)
	{
		super();
	...
	public override function press():Bool
	{
		...
		trace('Dialogue active: ' + FlxG.save.data.playDialogue);
		display = updateDisplay();
		return true;
	}

	private override function updateDisplay():String
		return "Dialogues " + (!FlxG.save.data.playDialogue ? "off" : "on");
}

class CutsceneControl extends Option
{
	public function new(desc:String)
	{
		super();
	...
	public override function press():Bool
	{
		FlxG.save.data.playCutscene = false;
		trace('Cutscene active: ' + FlxG.save.data.playCutscene);
		display = updateDisplay();
		return true;
	}

	private override function updateDisplay():String
		return "Cutscenes off for demo";
}

class HealthdrainControl extends Option
{
	public function new(desc:String)
	{
		super();
	...
	public override function press():Bool
	{
		FlxG.save.data.drainHealth = !FlxG.save.data.drainHealth;
		trace('Health drain: ' + FlxG.save.data.drainHealth);
		display = updateDisplay();
		return true;
	}

	private override function updateDisplay():String
		return "Health drain " + (!FlxG.save.data.drainHealth ? "off" : "on");
}