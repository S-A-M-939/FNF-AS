...
class Character extends FlxSprite
{
	...
	public var isDystopian:Bool = false;
	...
	public function new(x:Float, y:Float, ?character:String = 'gf', ?isPlayer:Bool = false)
	{
		...
		{
			case 'bf':
				tex = Paths.getSparrowAtlas('characters/GIRLFRIEND', 'shared');
				frames = tex;
			...
			// Dystopian
			case 'volcano':
				tex = Paths.getSparrowAtlas('characters/GF_dystopian', 'shared');
				frames = tex;

				animation.addByPrefix('idle', 'GFdystopian', 24, false);
			...
			case 'river':
				tex = Paths.getSparrowAtlas('characters/BF_dystopian', 'shared');
				frames = tex;

				animation.addByPrefix('idle', 'BFdystopian', 24, false);

				playAnim('idle');
				isDystopian = true;

			case 'tree':
				tex = Paths.getSparrowAtlas('characters/Olga_dystopian', 'shared');
				frames = tex;

				animation.addByPrefix('idle', 'OlgaDystopian', 24);
		...
		if (isPlayer)
		{
			...
			if (!curCharacter.startsWith('bf') && !isDystopian)
			{
				// var animArray
