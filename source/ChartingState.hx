...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	var playerTwoIcon:HealthIcon;
	var playerOneIcon:HealthIcon;
	...
	public var posLeft:FlxPoint;
	public var posRight:FlxPoint;
	...
	override function create()
	{
		...
		playerTwoIcon = new HealthIcon(_song.player2);
		playerOneIcon = new HealthIcon(_song.player1);
		playerTwoIcon.scrollFactor.set(1, 1);
		playerOneIcon.scrollFactor.set(1, 1);

		playerTwoIcon.setGraphicSize(0, 45);
		playerOneIcon.setGraphicSize(0, 45);

		add(playerTwoIcon);
		add(playerOneIcon);

		playerTwoIcon.setPosition(0, -100);
		playerOneIcon.setPosition(gridBG.width / 2, -100);

		posLeft = new FlxPoint(0, -100);
		posRight = new FlxPoint(gridBG.width / 2, -100);
	...
	function addSongUI():Void
	{
		...
		var gfVersions:Array<String> = CoolUtil.coolTextFile(Paths.txt('spectatorsList'));
		...
		var gfVersionLabel = new FlxText(10,180,64,'Spectator');
	...
	function updateHeads():Void
	{
		...
		{
			playerTwoIcon.setPosition(posRight.x, posRight.y);
			playerOneIcon.setPosition(posLeft.x, posLeft.y);
		}
		else
		{
			playerTwoIcon.setPosition(posLeft.x, posLeft.y);
			playerOneIcon.setPosition(posRight.x, posRight.y);
		}
	}
	...
	* Draw grid here
	...
	function updateGrid():Void
	{
		...
		for (i in sectionInfo)
		{
			...
			var animates = true;
			if (i[3] == true || i[3] == false) { animates = i[3]; }
	...
	function deleteNote(note:Note):Void
	{
		...
		// weird work around, just to get the notes to be correctly removed
		var realNoteData = note.x > 120 ? note.noteData + 4 : note.noteData;
		var noteX = realNoteData * GRID_SIZE;

		for (i in _song.notes[curSection].sectionNotes)
		{
			if (i[0] == note.strumTime && i[1] % 4 == note.noteData && (i[1] * GRID_SIZE == noteX))
			{
	...
	private function addNote(?n:Note):Void
	{
		...
		var noteAnimates = FlxG.keys.pressed.ALT == true ? false : true;
			
		var noteSus = 0;

		if (n != null)
			_song.notes[curSection].sectionNotes.push([n.strumTime, n.noteData, n.sustainLength, n.animatesChar]);
		else
			_song.notes[curSection].sectionNotes.push([noteStrum, noteData, noteSus, noteAnimates]);
